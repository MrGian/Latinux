...
public class LicenseChecker implements ServiceConnection {
    ...
    /**
     * Checks if the user should have access to the app. Binds the service if
     * necessary.
     * <p>
     * NOTE: This call uses a trivially obfuscated string (base64-encoded). For best
     * security, we recommend obfuscating the string that is passed into bindService
    ...
    public synchronized void checkAccess(LicenseCheckerCallback callback) {
        ...
        if (mPolicy.allowAccess()) {
            ...
            LicenseValidator validator = new LicenseValidator(mPolicy, new NullDeviceLimiter(), callback,
                    generateNonce(), mPackageName, mVersionCode);

            if (mService == null) {
                Log.i(TAG, "Binding to licensing service.");
                try {
                    boolean bindResult = mContext.bindService(new Intent(new String(
                            // Base64 encoded -
                    ...
                    callback.applicationError(LicenseCheckerCallback.ERROR_MISSING_PERMISSION, e.toString());
                } catch (Base64DecoderException e) {
                    e.printStackTrace();
                }
            } else {
    ...
    /**
     * Triggers the last deep link licensing URL returned from the server, which
     * redirects users to a page which enables them to gain access to the app. If no
     * such URL is returned by the server, it will go to the details page of the app
     * in the Play Store.
     */
